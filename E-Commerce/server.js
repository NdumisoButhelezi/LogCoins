...
const WALLET_ADDRESS = 'https://ilp.interledger-test.dev/zulu';
...
let storePaymentPointer = '$ilp.interledger-test.dev/zulu';
...
// Update store's payment pointer
app.post('/api/update-payment-pointer', async (req, res) => {
  const { paymentPointer } = req.body;
  if (paymentPointer) {
    storePaymentPointer = paymentPointer;
    res.json({ success: true, message: 'Payment pointer updated successfully' });
  } else {
    res.status(400).json({ success: false, message: 'Invalid payment pointer' });
  }
});
...
app.post('/api/open-payments-purchase', async (req, res) => {
  try {
    ...
    const sendingWalletAddress = await client.walletAddress.get({
      url: paymentPointer.replace('$', '$ilp.interledger-test.dev/zulu'),
    });
